...
export class FranceMapComponent implements OnInit {
  // Map postal codes to department names
  displayRegions = '';
  ...
  selectedRegions: { id: string; name: string; order: number }[] = [];
    ...
    if (target.classList.contains('land')) {
      ...
      if (index === -1) {
        this.selectedRegions.push({
          id: regionId,
          name: regionName,
          order: this.selectedRegions.length + 1,
        });
  ...
  private updateOrder(): void {
    this.selectedRegions = this.selectedRegions.map((region, index) => ({
      ...region,
      order: index + 1,
    }));
  ...
  saveRegions(): void {
    localStorage.setItem('selectedRegions', JSON.stringify(this.selectedRegions));
...
for (const region of this.selectedRegions) {
  this.displayRegions = this.displayRegions + region.name + ' -> ';

  }
  this.displayRegions = this.displayRegions.substring(0, this.displayRegions.length - 4);
}
}